...
const Category = ({ route}) => {
    ...
    const renderItem = ({ item }) => (
        <Item
            item={item}
            value={item}
            onPress={() => navigation.navigate('FoodItems', { items: item })}
        />
    );
    ...
    return (
        <ScrollView style={styles.container}>
            <View style={styles.header}>
                <TouchableOpacity activeOpacity={0.3} onPress={() => navigation.navigate('Menu')}>
                    <Icon name="arrow-back" size={30} color="#FDC913" />
                </TouchableOpacity>
                <Text style={styles.headertxt}>{item.name}</Text>
                <TouchableOpacity style={styles.inputButton} onPress={() => navigation.navigate('AddCategory')}>
